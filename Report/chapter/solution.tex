...
The dealer, event service and players will all be deployed on different hosts. In the implementation the event service is called PubSubServer. The different entities will then communicate through an UDP socket using topic-based messages. The player and dealer will subscribe to specific topics through the Publish/Subscribe server.
